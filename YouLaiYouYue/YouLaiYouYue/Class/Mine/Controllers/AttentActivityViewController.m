//
//  AttentActivityViewController.m
//  YouLaiYouYue
//
...
#import "AttentActivityViewController.h"
...
@interface AttentActivityViewController ()<UITableViewDelegate ,UITableViewDataSource>

@property (strong , nonatomic) UITableView *tableView;

@end

@implementation AttentActivityViewController {
...
- (void)setNavigationBar {
    [self setNavigationBarTitle:@"我关注的活动" textColor:UIColorFromRGB(0xffffff) titleFontSize:nil];
}
...
- (void)getDataFromUrl {
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_user/party_follow_attention",baseUrl];
...
- (NSMutableArray *)collectDataArray:(NSMutableArray *)param {
    ...
    for (NSDictionary *dic in param) {
        AttActModel *model = [AttActModel arrayWithDic:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath  {
    ...
    AttActModel *model = collectArray[indexPath.row];
    [cell.img sd_setImageWithURL:[NSURL URLWithString:model.pfpic] placeholderImage:nil];
    cell.timeLable.text = model.pfgotime;
    cell.contenLabel.text = model.pftitle;
    return cell;
}
