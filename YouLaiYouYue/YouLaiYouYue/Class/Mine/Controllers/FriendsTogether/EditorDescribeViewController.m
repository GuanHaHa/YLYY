//
//  EditorDescribeViewController.m
//  YouLaiYouYue
//
...
@interface EditorDescribeViewController ()<UITableViewDelegate , UITableViewDataSource ,EditorDescribeTableViewDelegate>

@property (strong , nonatomic) UITableView *tableView;

@end

@implementation EditorDescribeViewController {
...
- (void)rightButton:(UIButton *)button {
    ...
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_activity/edit_activity_img_info",baseUrl];
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:NO swpResultSuccess:^(id  _Nonnull resultObject) {
        if ([resultObject[@"code"] integerValue] == 200) {
            ...
            for (BaseViewController *controller in self.navigationController.viewControllers) {
                if ([controller isKindOfClass:[EditActivityViewController class]]) {
                    EditActivityViewController *vc = (EditActivityViewController *)controller;
...
- (void)getDataFromUrl {
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_activity/get_activity_img",baseUrl];
    NSString *md5Url = [self MD5ForLower32Bate:url];
    NSDictionary *dic = @{
                          @"app_key"  : md5Url,
