...
#import "StartActTableViewCell.h"
#import "StartFunctionTableViewCell.h"

#import "StartModel.h"

@interface StartActivityViewController ()<UITableViewDelegate , UITableViewDataSource, StartFunctionCellDelegate>
...
- (void)setNavigationBar {
    [self setNavigationBarTitle:@"发起的活动" textColor:UIColorFromRGB(0xffffff) titleFontSize:nil];
}
...
- (void)getDataFromUrl {
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_activity/activity_list",baseUrl];
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:NO swpResultSuccess:^(id  _Nonnull resultObject) {
        if ([resultObject[@"code"] integerValue] == 200) {
            ...
            [startArray addObjectsFromArray:[weakSelf startDataArray:resultObject[@"obj"]]];
...
- (NSMutableArray *)startDataArray:(NSMutableArray *)parma {
    NSMutableArray *array = [NSMutableArray array];
    for (NSDictionary *dic in parma) {
        StartModel *model = [StartModel arrayWithDic:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath  {
    if (indexPath.row == 0) {
        ...
        StartActTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"StartActCell"];
        cell.model = startArray[indexPath.section];
        return cell;
    }else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 0) {
        StartModel *model = startArray[indexPath.section];
...
- (void)selectStartFunctionCell:(NSInteger)index with:(NSInteger)Section{
    StartModel *model = startArray[Section];
    if (index == 0) {
        EditActivityViewController *vc = [[EditActivityViewController alloc] init];
        vc.activity_id = model.pfID;
        vc.pfexamine = model.pfexamine;
        [self.navigationController pushViewController:vc animated:YES];
        NSLog(@"%ld  %ld",(long)index,(long)Section);
    }
    if (index == 1) {
        NSLog(@"%ld  %ld",(long)index,(long)Section);
        if ([model.if_over isEqualToString:@"1"]) {
            NSLog(@"关闭群聊");
            [self closeTeamMeg:Section];
        }else {
...
// 关闭群聊
- (void)closeTeamMeg:(NSInteger )index {
    StartModel *model = startArray[index];
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_public/DisbandedGroup",baseUrl];
    NSString *md5Url = [self MD5ForLower32Bate:url];
    NSDictionary *dic = @{
                          @"app_key" : md5Url,
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:NO swpResultSuccess:^(id  _Nonnull resultObject) {
        if ([resultObject[@"code"] integerValue] == 200) {
            [LCProgressHUD showMessage:@"关闭成功"];
        }else {
...
- (void)cancelActivity:(NSString *)string withSection:(NSInteger )section {
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:NO swpResultSuccess:^(id  _Nonnull resultObject) {
        if ([resultObject[@"code"] integerValue] == 200) {
            ...
            [startArray removeObjectAtIndex:section];
            [weakSelf.tableView reloadData];
        }else {
            [LCProgressHUD showMessage:@"已有报名用户，无法取消"];
        }
    } swpResultError:^(id  _Nonnull resultObject, NSString * _Nonnull errorMessage) {
       [LCProgressHUD showMessage:@"已有报名用户，无法取消"];
    }];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[StartActTableViewCell class] forCellReuseIdentifier:@"StartActCell"];
