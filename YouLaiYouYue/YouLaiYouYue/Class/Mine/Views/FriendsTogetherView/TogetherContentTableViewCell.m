//
//  TogetherContentTableViewCell.m
//  YouLaiYouYue
//
...
#import "TogetherContentTableViewCell.h"

@implementation TogetherContentTableViewCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        placeHolderLabel.text = @"请输入内容(最多不超过2000字!!!)";
...
- (void)layoutSubviews {
    ...
    [_textView autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_titleLabel withOffset:10];
...
- (void)textViewDidEndEditing:(UITextView *)textView {
    if ([self.delegate respondsToSelector:@selector(TogetherChangeTextView:)]) {
        [self.delegate TogetherChangeTextView:textView.text];
    }
    NSLog(@"%@",textView.text);
}
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    {
        UIAlertView * alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"输入上限2000字" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alert show];
        return NO;
    }
    else
