...
@interface InsertImgTableViewCell ()<HXPhotoViewDelegate>

@end

@implementation InsertImgTableViewCell
...
- (void)setModel:(MoreImgModel *)model {
    _model = model;
...
- (void)photoView:(HXPhotoView *)photoView changeComplete:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photos videos:(NSArray<HXPhotoModel *> *)videos original:(BOOL)isOriginal {
    ...
    if ([self.delegate respondsToSelector:@selector(changePhoto:with:)]) {
        [self.delegate changePhoto:photoView with:allList];
    }
...
- (void)setImgCount:(NSInteger)imgCount {
    ...
    _manager.configuration.photoMaxNum = 9-imgCount;
