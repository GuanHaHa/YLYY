...
-(instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        self.alertView = [[UIView alloc] initWithFrame:CGRectMake(75, (kScreen_Height-alertViewHeight)/2.0, kScreen_Width-150, alertViewHeight)];
        ...
        self.selectTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.titleLabel.frame), kScreen_Width-150, self.alertView.bounds.size.height-buttonHeight*2) style:UITableViewStylePlain];
        ...
        [self.confirmButton addTarget:self action:@selector(cancelAction) forControlEvents:UIControlEventTouchUpInside];
        ...
        [self.cancelButton addTarget:self action:@selector(confirmAction) forControlEvents:UIControlEventTouchUpInside];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.type == 1) {
        HeadlineModel *model = _datas[indexPath.row];
        cell.titleLabel.text = model.pftitle;
    }else if (self.type == 2){
        TagModel *model = _datas[indexPath.row];
        cell.titleLabel.text = model.lname;
    }else if (self.type == 3) {
        TitleModel *model = _datas[indexPath.row];
        cell.titleLabel.text = model.pftitle;
    }else if (self.type == 4) {
        EditInserModel *model = _datas[indexPath.row];
        cell.titleLabel.text = model.fftitle;
    }
...
- (void)confirmAction{
    if (self.ifSupportMultiple == NO) {
        if (self.type == 1) {
            HeadlineModel *model = self.datas[self.selectIndexPath.row];
            if (_delegate && [_delegate respondsToSelector:@selector(singleChoiceBlockData:withDataID:withType:)]) {
                [_delegate singleChoiceBlockData:model.pftitle withDataID:model.pfID withType:@""];
            }
            
        }else if (self.type == 2){
            TagModel *model = self.datas[self.selectIndexPath.row];
            if (_delegate && [_delegate respondsToSelector:@selector(singleChoiceBlockData:withDataID:withType:)]) {
                [_delegate singleChoiceBlockData:model.lname withDataID:model.lID withType:@""];
            }
            
        }else if (self.type == 3) {
            TitleModel *model = self.datas[self.selectIndexPath.row];
            if (_delegate && [_delegate respondsToSelector:@selector(singleChoiceBlockData:withDataID:withType:)]) {
                [_delegate singleChoiceBlockData:model.pftitle withDataID:model.pfID withType:model.block];
            }
        }else if (self.type == 4) {
            EditInserModel *model = self.datas[self.selectIndexPath.row];
            if (_delegate && [_delegate respondsToSelector:@selector(singleChoiceBlockData:withDataID:withType:)]) {
                [_delegate singleChoiceBlockData:model.fftitle withDataID:model.ffID withType:@""];
            }
        }
