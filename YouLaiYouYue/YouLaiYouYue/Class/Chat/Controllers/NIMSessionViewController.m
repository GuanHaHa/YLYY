...
- (BOOL)onTapCell:(NIMKitEvent *)event{
    ...
    NIMMessageType messtype = event.messageModel.message.messageType;
    NSLog(@"messtype-%ld",(long)messtype);
    
    if (messtype == 3)
    {
        NSDictionary *dicc = (NSDictionary *)event.messageModel.message.messageObject;
        NSLog(@"%@",dicc);
        NSLog(@"%@",[dicc valueForKey:@"url"]);
        [self lookVideo:[dicc valueForKey:@"url"]];
        handle = YES;
    }
    if (messtype == NIMMessageTypeImage)
    {
        NSDictionary *dicc = (NSDictionary *)event.messageModel.message.messageObject;
        NSLog(@"%@",[dicc valueForKey:@"url"]);
        [self lookPic:[dicc valueForKey:@"url"]];
        handle = YES;
    }
...
-(void)lookVideo:(NSString *)sender{
    LookVideoViewController *videoVC =[[LookVideoViewController alloc] init];
    videoVC.urlStr = sender;
    [self presentViewController:videoVC animated:YES completion:^{
        
    }];
    
}
-(void)lookPic:(NSString *)sender{
    LookPicViewController *videoVC =[[LookPicViewController alloc] init];
    videoVC.urlStr = sender;
    [self presentViewController:videoVC animated:YES completion:^{
