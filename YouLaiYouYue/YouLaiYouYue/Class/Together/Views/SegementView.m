...
- (void)calculatorWidths{
    ...
    // 计算所有标题的宽度
    for (TagModel *model in self.titles) {
        
        NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithObject:[UIFont systemFontOfSize:15] forKey:NSFontAttributeName];
        CGSize size = [model.lname boundingRectWithSize:CGSizeMake(MAXFLOAT, 0.0) options:NSStringDrawingUsesLineFragmentOrigin attributes:dic context:nil].size;
...
- (void)setUpSegmentView{
    ...
    for (int i = 0; i < self.titles.count; i++) {
        TagModel *model = self.titles[i];
        UILabel *label = [[UILabel alloc] init];
        label.userInteractionEnabled = YES;
        label.text = model.lname;
        label.tag = i;
...
- (void)titleClick:(UITapGestureRecognizer *)tap{
    ...
    if ([self.delegate respondsToSelector:@selector(didSelectWithIndex:)]) {
        [self.delegate didSelectWithIndex:i];
    }
...
- (void)selectLabel:(UILabel *)label
{
    ...
    // 修改标题选中颜色
    label.textColor = UIColorFromRGB(0xff9d00);
...
- (UIView *)underLine
{
    if (_underLine == nil) {
        ...
        underLineView.backgroundColor = _underLineColor?_underLineColor:UIColorFromRGB(0xff9d00);
