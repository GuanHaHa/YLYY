...
- (void)layoutSubviews {
    ...
    [_titleLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_bigImg withOffset:20];
    [_titleLabel autoPinEdgeToSuperviewEdge:ALEdgeRight withInset:15];
    [_titleLabel autoSetDimension:ALDimensionHeight toSize:40];
    
    [_lookImg autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_titleLabel withOffset:15];
    ...
    [_lookLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_titleLabel withOffset:15];
    ...
    [_concernImg autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_titleLabel withOffset:15];
    ...
    [_concernLabel autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_titleLabel withOffset:15];
    ...
    [_headImg autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_lookLabel withOffset:15];
    ...
    [_infoView autoPinEdge:ALEdgeTop toEdge:ALEdgeBottom ofView:_headImg withOffset:10];
    [_infoView autoPinEdgeToSuperviewEdge:ALEdgeLeft withInset:0];
    [_infoView autoPinEdgeToSuperviewEdge:ALEdgeRight withInset:0];
    [_infoView autoPinEdgeToSuperviewEdge:ALEdgeBottom withInset:0];
    
}

- (void)setModel:(ActivityModel *)model {
    _model = model;
    if ([_model.captain isEqualToString:GetUserDefault(userID)]) {
        _zanBtn.hidden = YES;
    }else {
        _zanBtn.hidden = NO;
    }
    if ([_model.attention_captain isEqualToString:@"0"]) {
        _zanBtn.selected = NO;
    }else {
        _zanBtn.selected = YES;
    }
    _nameLabel.text = _model.capttain_name;
    [_bigImg sd_setImageWithURL:[NSURL URLWithString:_model.show_pic] placeholderImage:nil];
    [_headImg sd_setImageWithURL:[NSURL URLWithString:_model.capttain_pic] placeholderImage:nil];
    _titleLabel.text = _model.title;
    _lookLabel.text = [NSString stringWithFormat:@"%@浏览",_model.look];
    _concernLabel.text = [NSString stringWithFormat:@"%@关注",_model.pffavorite];
    _lvLabel.text = [NSString stringWithFormat:@"LV%@",_model.level];
    _infoView.label1.text = [NSString stringWithFormat:@"开始时间:%@",_model.begin_time];
    _infoView.label2.text = [NSString stringWithFormat:@"结束时间:%@",_model.end_time];
    _infoView.label3.text = [NSString stringWithFormat:@"活动地点:%@",_model.city];
    _infoView.label4.text = [NSString stringWithFormat:@"参加费用:%@",_model.price];
    _infoView.label5.text = [NSString stringWithFormat:@"要求单身:%@",_model.marry];
    _infoView.label6.text = [NSString stringWithFormat:@"性别要求:%@",_model.peoplesex];
    _infoView.label7.text = [NSString stringWithFormat:@"年龄要求:%@岁",_model.age];
    if ([_model.pfexplain isEqualToString:@""]) {
        _model.pfexplain = @"无";
    }
    _infoView.label8.text = [NSString stringWithFormat:@"其他要求:%@",_model.pfexplain];
    if ([_model.if_sign isEqualToString:@"0"]) {
...
- (UIImageView *)concernImg {
    if (!_concernImg) {
        ...
        _concernImg.contentMode = UIViewContentModeScaleAspectFit;
    }
    return _concernImg;
}
...
- (InfoView *)infoView {
    if (!_infoView) {
        _infoView = [[InfoView alloc] initForAutoLayout];
