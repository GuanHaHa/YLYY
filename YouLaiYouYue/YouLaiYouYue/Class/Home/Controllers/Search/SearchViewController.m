...
- (void)viewDidLoad {
    ...
    [self getHotSeach];
    [self setNavigationBarTitle:@"" textColor:UIColorFromRGB(0xf8f8f8) titleFontSize:nil];
    self.searchHistoriesCount = 20;
    
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-JCNew64) style:UITableViewStylePlain];
    ...
    // 创建搜索框
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(10, 7, SCREEN_WIDTH-64-20-20-50, 30)];
...
- (void)getHotSeach {
    NSString *url = [NSString stringWithFormat:@"%@index.php/action/ac_public/all_search",baseUrl];
    ...
    [self swpPublicTooGetDataToServer:url parameters:dic isEncrypt:NO swpResultSuccess:^(id  _Nonnull resultObject) {
        if ([resultObject[@"code"] integerValue] == 200) {
            for (NSDictionary *dci in resultObject[@"obj"]) {
                [weakSelf.tagsArray addObject:dci[@"title"]];
            }
            [weakSelf tagsViewWithTag];
...
- (void)tagDidCLick:(UITapGestureRecognizer *)gr
{
    ...
    //创建一个消息对象
    NSNotification * notice = [NSNotification notificationWithName:@"searchBarDidChange" object:nil userInfo:@{@"searchText":label.text,@"type":self.type}];
    //发送消息
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.view bringSubviewToFront:self.searchSuggestionVC.view];
    //创建一个消息对象
    NSNotification * notice = [NSNotification notificationWithName:@"searchBarDidChange" object:nil userInfo:@{@"searchText":cell.textLabel.text,@"type":self.type}];
    //发送消息
    [[NSNotificationCenter defaultCenter]postNotification:notice];
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    {
        self.searchSuggestionVC.tableView.hidden = NO;
        self.tableView.hidden = YES;
        [self.view bringSubviewToFront:self.searchSuggestionVC.tableView];
        
        //创建一个消息对象
        NSNotification * notice = [NSNotification notificationWithName:@"searchBarDidChange" object:nil userInfo:@{@"searchText":searchText,@"type":self.type}];
        //发送消息
        [[NSNotificationCenter defaultCenter]postNotification:notice];
    }
